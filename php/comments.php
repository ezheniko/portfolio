...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $amount = (int) $clientParam['amount'];
  $current = $data['amount'][0] - (int) $clientParam['startPosition'] - $amount;
  if ($current < 0) {
    $amount += $current;
    $current = 0;
  }
  ...
  $amount = abs($amount);
  $sth = $dbh->query("SELECT text, time, autor FROM comments LIMIT {$current}, {$amount}");
